...
function LlfTable({
  ...
  jobs: LLFJob[];
  onUpdate: (id: string, field: keyof LLFJob, value: number | "") => void;
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell>
              <Tooltip title="Deadline (D): relativni rok; apsolutni = A + D.">
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  Deadline (D){" "}
                  <InfoOutlinedIcon fontSize="small" color="action" />
                </Box>
...
/* ---------- Stranica: LLF ---------- */
export default function Llf() {
  const [jobs, setJobs] = useState<LLFJob[]>([]);
      ...
      <Dialog open={openInfo} onClose={() => setOpenInfo(false)} maxWidth="md">
        ...
        <DialogContent style={{ padding: "10px 40px" }}>
          <Typography gutterBottom>
            LLF u svakom trenutku bira posao s{" "}
            <strong>najmanjom laksnošću</strong>: ℓ(t) = (A + D) − t −
            preostalo. Algoritam je preemptivan – čim neki drugi ima manju ℓ,
            preuzima CPU. U ovoj stranici uređuješ C, D i A.
          </Typography>
        </DialogContent>
      </Dialog>
    </Box>
