...
/* ---------- Tip posla za SCHED_SPORADIC (C,D,T + A + boja) ---------- */
type SPJob = {
  id: string;
  name: string;
  runtime: number | ""; // C
  deadline: number | ""; // D
  period: number | ""; // T (min inter-arrival / server period)
...
function SporadicTable({
  ...
  jobs: SPJob[];
  onUpdate: (id: string, field: keyof SPJob, value: number | "") => void;
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell>
              <Tooltip title="Runtime (C): maksimalno CPU vrijeme za zahtjev.">
            ...
            <TableCell>
              <Tooltip title="Deadline (D): rok dovršetka zahtjeva.">
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  Deadline (D){" "}
                  <InfoOutlinedIcon fontSize="small" color="action" />
                </Box>
            ...
            <TableCell>
              <Tooltip title="Period (T): minimalni međudolasak / replenishment period.">
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  Period (T){" "}
                  <InfoOutlinedIcon fontSize="small" color="action" />
                </Box>
            ...
            <TableCell>
              <Tooltip title="Arrival (A): vrijeme dolaska zahtjeva.">
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  Arrival (A){" "}
                  <InfoOutlinedIcon fontSize="small" color="action" />
                </Box>
...
/* ---------- Stranica: SCHED_SPORADIC ---------- */
export default function SchedSporadic() {
  const [jobs, setJobs] = useState<SPJob[]>([]);
      ...
      <Dialog open={openInfo} onClose={() => setOpenInfo(false)} maxWidth="md">
        ...
        <DialogContent style={{ padding: "10px 40px" }}>
          <Typography gutterBottom>
            <strong>Sporadični</strong> (aperiodični) zahtjevi tipično se
            poslužuju EDF-om ili preko Sporadic Servera (budget + replenishment
            period). U ovoj aplikaciji svaki zahtjev ima <em>C, D, T, A</em>, a
            simulacija je pojednostavljena (one-shot) radi jasnog prikaza.
          </Typography>
        </DialogContent>
      </Dialog>
    </Box>
